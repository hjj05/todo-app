...
class _TodoScreenState extends State<TodoScreen> {
  ...
  void _deleteTask(int index) {
    setState(() {
      taskData[selectedDate] = List.from(tasks)..removeAt(index);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("${selectedDate.day} ${_getMonth(selectedDate.month)}"),
        actions: [
          IconButton(
...
class TaskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      color: isDone ? Colors.purple[100] : Colors.grey[200],
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: ListTile(
        leading: Checkbox(value: isDone, onChanged: onChanged, activeColor: Colors.purple),
        title: Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        subtitle: Text("$time\n$desc", style: const TextStyle(fontSize: 14)),
        isThreeLine: true,
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
